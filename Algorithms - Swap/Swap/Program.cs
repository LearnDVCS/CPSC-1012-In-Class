...
namespace Swap
{
    class Program
    {
        ...
        private static void DemoObjectSwapMethods()
        {
            ...
            Console.WriteLine($"Results of GoodSwap method: {varA} and {varB}");
            Console.WriteLine("Attempting to swap *contents* of objects:");
            ObjectContentSwap(varA, varB);
        }
        private static void ObjectContentSwap(Person a, Person b)
        {
            // A content swap doesn't change the reference variable, just it's contents (properties)
            string temp;
            temp = a.Name; // I drill into the Person object
            a.Name = b.Name; // drilling in again
            b.Name = temp;
        }
